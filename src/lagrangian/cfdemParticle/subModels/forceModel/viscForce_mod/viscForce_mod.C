...
defineTypeNameAndDebug(viscForce_mod, 0);
...
:
    ...
    neeta_(sm.mesh().lookupObject<volScalarField>("neeta")),
    addedMassCoeff_(0.0)
{
    // block viscForce_modModel for type B
    if (modelType_ == "B") FatalError <<"using  model viscForce_mod with model type B is not valid\n" << abort(FatalError);
    ...
    if(probeIt_)
    {
        ...
        particleCloud_.probeM().vectorFields_.append("viscForce_mod"); //first entry must the be the force
    ...
    if (propsDict_.found("useAddedMass")) 
    {
        ...
        Info << "viscForce_mod will also include added mass with coefficient: " << addedMassCoeff_ << endl;
...
void viscForce_mod::setForce() const
{
	...
	neeta = neeta_[cellI];
        if ((cellI > -1) && (neeta ==0)) // particle Found
        {
