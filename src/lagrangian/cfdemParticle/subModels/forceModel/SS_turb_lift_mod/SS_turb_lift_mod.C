...
defineTypeNameAndDebug(SS_turb_lift_mod, 0);
...
SS_turb_lift_mod::SS_turb_lift_mod
(
    ...
    useSecondOrderTerms_(false), sm_ (sm)
{
    ...
    particleCloud_.probeM().vectorFields_.append("lift_modForce"); //first entry must the be the force
...
void SS_turb_lift_mod::setForce() const
{
    ...
    for(int index = 0;index <  particleCloud_.numberOfParticles(); index++)
    {
        ...
        //{
            ...
            Foam::vector position_up (position[0], position[1]+ds, position[3]);
            Foam::vector position_dn (position[0], position[1]-ds, position[3]);
            ...
            int cellI2   = searchEngine_.findCell(position,-1,treeSearch_);///////////////////
            int foundIndex = -1;
            for (int i = 0; i < particleCloud_.numberOfParticles(); ++i) 
            {
            if (particleCloud_.cellIDs()[i][0] == cellI2) 
                {
                   foundIndex = i;
                   break; 
                }
            }
            //Pout << "cell2 = " << cellI2 << endl;
	        //Pout << "found = " << particleCloud_.cellIDs()[foundIndex][0] << endl;
            ...
            if (cellI > -1) // particle Found
            {
                ...
                if (magUr > 0)
                {
                    ...
                    scalar U_star_old = U_star;
                    drag = dragCoefficient*Ur;
                    if (gradAlpha_ch > 0.5)
                    {
                        U_star      = 0.0001;//abs(0.001*sqrt(tau/rho));
                        //U_star = 0.001;
                        //Pout << "Kspl = " << Kspl << endl;
                        //Pout << "Yplus = " << Yplus << endl;
                        if (Yplus >= 5)
                        {
                            for (int i = 0; i < 20; ++i) 
                            {
                                ...
                                //scalar turb = (1 / 0.41) * std::log(1+((0.5 * ds*2.5 * U_star) / nuf));
                                //Pout << "term3 = " << term3 << endl;
                                //Pout << "larger = " << turb << endl;
                                ...
                                U_star = abs(U_star_new);
                                //Yplus   = yDim*rho*U_star/nuf;
                                //Kspl = rho*2.5*ds*U_star/nuf;
                            }
